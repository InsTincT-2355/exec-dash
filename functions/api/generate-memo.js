...
function createFallbackMemo(tasks, requesterName) {
  ...
  const deptLabel = departments.join(", ") || "Selected Departments";
  return cleanMemoText([
    "To: Executive Leadership",
    `From: ${requesterName}`,
    `Subject: Weekly Executive Memo - ${deptLabel}`,
    "",
    "Summary",
    `This memo covers ${tasks.length} selected update items across ${deptLabel}.`,
    "",
    "Key Highlights",
    formatMemoList(
...
async function callAiProvider(env, tasks, requesterName) {
  ...
  if (provider === "gemini") {
    ...
    return cleanMemoText(
      payload?.candidates?.[0]?.content?.parts?.map((part) => part.text || "").join("").trim()
        || createFallbackMemo(normalizedTasks, requesterName),
      requesterName
    );
  }
